...
function inspectPages() {
  ...
  const targetUuids: Record<number, string> = {
  ...
  for (const [num, uuid] of Object.entries(targetUuids)) {
    ...
    fs.writeFileSync(path.join(process.cwd(), `extracted/page_${num}_inner.html`), pageHtml, 'utf8')
    console.log(`Saved page ${num} (length ${pageHtml.length})`)
  }
}
